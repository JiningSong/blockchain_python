...
def verify_chain():
    ...
    for (index, block) in enumerate(blockchain):
        ...
        if block['previous_hash'] != hash_block(blockchain[index - 1]):
            return False
        #[:-1] exclude the reward transaction, since it is added after calculating the hash
...
while waiting_for_input:
    ...
    if user_choice == '1':
        ...
        if len(blockchain) >= 1:
            blockchain[0] = {
                'previous_hash': '',
                'index': 0,
                'transactions': [{'sender': 'Chris', 'recipient': 'Jining', 'amount': 100.0}]
            }
    elif user_choice == 'q':
    ...
    print('Balance of {}: {:6.2f}'.format('Jining', get_balance('Jining')))
else:
    print('User left!')
